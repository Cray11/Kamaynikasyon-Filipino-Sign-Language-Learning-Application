...
class SpellingSequenceSelectionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupLevelsRecycler()
        loadStats()
        maybeShowSpellingSequenceTutorial()
    }
            ...
            lifecycleScope.launch {
                ...
                try {
                    ...
                    val progress = withContext(Dispatchers.IO) { 
                        db.spellingSequenceProgressDao().get("level$levelToShow") 
                    }
                    showLevelInfoDialog(levelToShow, "spelling_sequence", progress)
                } catch (_: Exception) {
                    showLevelInfoDialog(levelToShow, "spelling_sequence", null)
                }
            }
        }
    }
    ...
    private fun setupToolbar() {
        ...
        toolbar.title = "Spelling Sequence Levels"
        ...
        findViewById<android.widget.TextView>(R.id.header_title)?.text = "Spelling Sequence"
        findViewById<android.widget.TextView>(R.id.header_subtitle)?.text = "Select a level to begin"
        
        // Update stats card stroke color to match Spelling Sequence theme
        val themeColor = android.graphics.Color.parseColor("#00828E")
    ...
    private fun setupLevelsRecycler() {
        ...
        val indexPath = "minigames/spelling_sequence/index.json"
        // Theme color for spelling sequence (matches stats card)
        val themeColor = android.graphics.Color.parseColor("#00828E")
        ...
        lifecycleScope.launch {
            ...
            recycler.adapter = PictureQuizLevelAdapter(sortedItemsForAdapter, { levelNumber ->
                lifecycleScope.launch {
                    try {
                        val db = AppDatabase.getInstance(applicationContext)
                        val progress = withContext(Dispatchers.IO) { db.spellingSequenceProgressDao().get("level$levelNumber") }
                        showLevelInfoDialog(levelNumber, "spelling_sequence", progress)
                    } catch (_: Exception) {
                        showLevelInfoDialog(levelNumber, "spelling_sequence", null)
                    }
                }
            }, themeColor)
            ...
            try {
                ...
                val rows = withContext(Dispatchers.IO) { db.spellingSequenceProgressDao().getAll() }
                ...
                recycler.adapter = PictureQuizLevelAdapter(updated, { levelNumber ->
                    lifecycleScope.launch {
                        showLevelInfoDialog(levelNumber, "spelling_sequence", byLevel["level$levelNumber"])
                    }
                }, themeColor)
            } catch (_: Exception) {
    ...
    private fun loadStats() {
        lifecycleScope.launch {
            try {
                val db = com.example.kamaynikasyon.data.database.AppDatabase.getInstance(applicationContext)
                val progress = withContext(Dispatchers.IO) { db.spellingSequenceProgressDao().getAll() }
                ...
                try {
                    val indexPath = "minigames/spelling_sequence/index.json"
                    val jsonString = String(assets.open(indexPath).use { it.readBytes() }, Charset.forName("UTF-8"))
                    val root = JSONObject(jsonString)
                    val levels = root.getJSONArray("levels")
                ...
                android.util.Log.e("SpellingSequenceSelection", "Error loading stats", e)
            }
        }
    }
    ...
    private fun maybeShowSpellingSequenceTutorial() {
        if (com.example.kamaynikasyon.core.utils.UserPrefs.shouldShowSpellingSequenceTutorial(this)) {
            showTutorial()
        }
    }
    ...
    private fun showTutorial() {
        ...
        val dialog = TutorialDialog(this, pages) { dontShowAgain ->
            if (dontShowAgain) {
                UserPrefs.setShowSpellingSequenceTutorial(this, false)
            }
        }
        dialog.show()
    }
    ...
    private fun loadTutorialPagesFromAsset(tutorialKey: String): List<TutorialPage> {
        ...
        try {
            ...
            android.util.Log.e("SpellingSequenceSelection", "Error loading tutorial", e)
        }
    ...
    private suspend fun showLevelInfoDialog(
        levelNumber: Int,
        minigameType: String,
        progress: com.example.kamaynikasyon.data.database.SpellingSequenceProgress?
    ) {
        ...
        val levelPath = "minigames/spelling_sequence/$levelId.json"
        var lettersCount = 0
        ...
        btnStart.setOnClickListener {
            dialog.dismiss()
            val intent = Intent(this, SpellingSequenceActivity::class.java)
    ...
    private suspend fun loadIndexData(assetPath: String): String? = withContext(Dispatchers.IO) {
        val indexPath = "spelling_sequence/index.json"
        
        // If user opted to use offline assets only, skip Supabase and cache
        val useOfflineAssetsOnly = ContentSyncManager.isUseOfflineAssetsOnly(this@SpellingSequenceSelectionActivity)
        if (useOfflineAssetsOnly) {
            return@withContext try {
                val jsonString = loadFromAssets(assetPath)
                android.util.Log.d("SpellingSequenceSelection", "Loaded index from assets (offline assets only mode)")
                jsonString
            } catch (e: Exception) {
                android.util.Log.e("SpellingSequenceSelection", "Failed to load index from assets", e)
                null
            }
        }
        
        val isOnline = ErrorHandler.isOnline(this@SpellingSequenceSelectionActivity)
        ...
        if (SupabaseConfig.isInitialized() && isOnline) {
            try {
                val jsonString = SupabaseStorage.downloadTextFile(
                    this@SpellingSequenceSelectionActivity,
                    SupabaseConfig.BUCKET_MINIGAMES,
                    indexPath
                )
                if (jsonString != null) {
                    // Cache to persistent storage for offline use
                    CacheManager.cacheData(this@SpellingSequenceSelectionActivity, "minigames", "spelling_sequence_index", jsonString)
                    android.util.Log.d("SpellingSequenceSelection", "Loaded index from Supabase and cached")
                    return@withContext jsonString
                }
            } catch (e: Exception) {
                android.util.Log.w("SpellingSequenceSelection", "Failed to load index from Supabase, trying cache", e)
            }
        }
        ...
        if (!isOnline || !SupabaseConfig.isInitialized()) {
            try {
                ...
                if (cacheFile.exists() && cacheFile.isFile) {
                    ...
                    android.util.Log.d("SpellingSequenceSelection", "Loaded index from SupabaseStorage cache (offline)")
                    return@withContext jsonString
                }
            } catch (e: Exception) {
                android.util.Log.w("SpellingSequenceSelection", "Failed to load from SupabaseStorage cache, trying CacheManager", e)
            }
            ...
            try {
                val cachedData = CacheManager.getCachedData(
                    this@SpellingSequenceSelectionActivity,
                    "minigames",
                    "spelling_sequence_index",
                    String::class.java
                )
                if (cachedData != null) {
                    android.util.Log.d("SpellingSequenceSelection", "Loaded index from CacheManager (offline)")
                    return@withContext cachedData
                }
            } catch (e: Exception) {
                android.util.Log.w("SpellingSequenceSelection", "Failed to load from CacheManager", e)
            }
        }
        ...
        return@withContext try {
            ...
            if (jsonString != null) {
                ...
                CacheManager.cacheData(this@SpellingSequenceSelectionActivity, "minigames", "spelling_sequence_index", jsonString)
                android.util.Log.d("SpellingSequenceSelection", "Loaded index from assets and cached")
            }
            jsonString
        } catch (e: Exception) {
            android.util.Log.e("SpellingSequenceSelection", "Failed to load index from assets", e)
    ...
    private suspend fun loadLevelDataForDialog(levelId: String, assetPath: String): String? = withContext(Dispatchers.IO) {
        val levelPath = "spelling_sequence/$levelId.json"
        ...
        if (SupabaseConfig.isInitialized() && isOnline) {
            try {
                ...
                android.util.Log.w("SpellingSequenceSelection", "Failed to load level from Supabase: $levelId, trying cache", e)
            }
        }
