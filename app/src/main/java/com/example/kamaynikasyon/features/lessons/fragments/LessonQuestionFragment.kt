...
import com.example.kamaynikasyon.databinding.FragmentLessonQuestionBinding
import com.example.kamaynikasyon.features.lessons.data.models.*
import com.google.android.material.button.MaterialButton
import androidx.core.view.children

class LessonQuestionFragment : Fragment() {
    
    private var _binding: FragmentLessonQuestionBinding? = null
    ...
    companion object {
        ...
        fun newInstance(page: LessonPage): LessonQuestionFragment {
            val fragment = LessonQuestionFragment()
            val args = Bundle()
    ...
    override fun onCreateView(
        ...
        _binding = FragmentLessonQuestionBinding.inflate(inflater, container, false)
        return binding.root
    }
     ...
     */
    private fun preloadVideos(question: com.example.kamaynikasyon.features.lessons.data.models.Question) {
                    ...
                    viewLifecycleOwner.lifecycleScope.launch {
                        try {
                            ...
                            if (videoUri == null || videoUri.toString().isEmpty()) {
                                android.util.Log.w("LessonQuestionFragment", "Invalid video URI for preload: ${media.path}")
                                return@launch
                            }
                            ...
                            player.addListener(object : Player.Listener {
                                override fun onPlayerError(error: PlaybackException) {
                                    android.util.Log.e("LessonQuestionFragment", "ExoPlayer error during preload: ${error.message}", error)
                                    ...
                                    try {
                                        ...
                                        if (assetUri.toString() != videoUri.toString()) {
                                            try {
                                                ...
                                                android.util.Log.d("LessonQuestionFragment", "Preloaded video from asset URI: ${media.path}")
                                            } catch (e: Exception) {
                                                android.util.Log.e("LessonQuestionFragment", "Error preloading video from asset: ${media.path}", e)
                                            }
                                        }
                                    } catch (e: Exception) {
                                        android.util.Log.e("LessonQuestionFragment", "Error handling player error", e)
                                    }
                                }
                            })
                            ...
                            android.util.Log.d("LessonQuestionFragment", "Preloaded video: ${media.path} from URI: $videoUri")
                            ...
                            try {
                                ...
                                player.prepare()
                                preloadedPlayers[key] = player
                                android.util.Log.d("LessonQuestionFragment", "Preloaded video from asset (fallback): ${media.path}")
                            } catch (fallbackError: Exception) {
                                android.util.Log.e("LessonQuestionFragment", "Error preloading video from asset (fallback): ${media.path}", fallbackError)
                            }
                        }
                    }
                }
    ...
    private fun setupQuestionContent(questionContent: QuestionContent) {
        ...
        // Set question text
        binding.tvQuestion.text = questionContent.text
    ...
    private fun setupAnswerOptions(options: List<AnswerOption>) {
    ...
    private fun setupVerticalAnswerOptions(options: List<AnswerOption>) {
        // Show vertical layout, hide grid layout
    ...
    private fun setupGridAnswerOptions(options: List<AnswerOption>) {
        ...
        data class OptionViews(
            val container: View,
            val videoFrameLayout: View,
            val videoView: View,
            val imageView: View,
            val textView: View
        )
        ...
        options.forEachIndexed { index, option ->
            if (index < optionContainers.size) {
                val optionViews = optionContainers[index]
                
                // Reset visibility
                optionViews.videoFrameLayout.visibility = View.GONE
                optionViews.videoView.visibility = View.GONE
                optionViews.imageView.visibility = View.GONE
                ...
                when (option.type) {
                    ContentType.VIDEO -> {
                        option.primaryMedia()
                            ...
                            ?.let { media ->
                                try {
                                    optionViews.videoFrameLayout.visibility = View.VISIBLE
                                    optionViews.videoView.visibility = View.VISIBLE
                                    initializePlayerForView(optionViews.videoView as PlayerView, media)
                                    // Show menu button and set click listener
                                    val menuButtonId = when (index) {
                        ...
                        (optionViews.textView as android.widget.TextView).text = option.text ?: "Video Option ${index + 1}"
                    }
                    ContentType.IMAGE -> {
                        option.primaryMedia()
                            ?.takeIf { it.isImage() }
                            ?.let { media ->
                                try {
                                    optionViews.imageView.visibility = View.VISIBLE
                                    (optionViews.imageView as android.widget.ImageView).setImageURI(media.asUri())
                                } catch (e: Exception) {
                                    showError(R.string.error_loading_image)
                                    optionViews.imageView.visibility = View.GONE
                                }
                            }
                        (optionViews.textView as android.widget.TextView).text = option.text ?: "Image Option ${index + 1}"
                    }
                    ContentType.TEXT -> {
                        (optionViews.textView as android.widget.TextView).text = option.text ?: "Option ${index + 1}"
                    }
                }
                
                optionViews.container.setOnClickListener {
                    handleOptionSelection(index)
                }
            }
        }
        ...
        try {
            ...
            if (selectedIndex < videoViews.size) {
                ...
                if (selectedVideoView.visibility == View.VISIBLE && selectedVideoView is PlayerView) {
                    ...
                    if (player == null && currentPage?.question != null && selectedIndex < currentPage!!.question!!.options.size) {
                        val option = currentPage!!.question!!.options[selectedIndex]
    ...
    private fun showAnswerFeedback(isCorrect: Boolean) {
        val optionContainers = if (binding.gridOptionsContainer.visibility == View.VISIBLE) {
            listOf(
                binding.gridOption1Container,
                binding.gridOption2Container,
                binding.gridOption3Container,
        ...
        optionContainers.forEachIndexed { index, view ->
            val resId = when {
                index == correctAnswer -> R.drawable.bg_answer_btn_correct
                index == selectedAnswer && !isCorrect -> R.drawable.bg_answer_btn_wrong
                else -> R.drawable.bg_answer_btn
            }
    ...
    private suspend fun trySupabaseThenAsset(media: MediaResource): android.net.Uri? = withContext(Dispatchers.IO) {
        ...
        if (media.isVideo() && SupabaseConfig.isInitialized()) {
            try {
                ...
                if (cachedPath != null) {
                    ...
                    android.util.Log.d("LessonQuestionFragment", "Using downloaded Supabase video: $fileUri")
                    return@withContext fileUri
                }
            } catch (e: Exception) {
                android.util.Log.w("LessonQuestionFragment", "Failed to download video from Supabase, trying URL or asset", e)
            }
        }
        ...
        if (supabaseUri != null) {
            ...
            if (uriString.startsWith("http://") || uriString.startsWith("https://")) {
                android.util.Log.d("LessonQuestionFragment", "Using Supabase URL: $uriString")
        ...
        val assetUri = media.asUri()
        android.util.Log.d("LessonQuestionFragment", "Using asset URI: $assetUri")
                ...
                try {
                    ...
                    viewLifecycleOwner.lifecycleScope.launch {
                        try {
                            ...
                            if (videoUri == null || videoUri.toString().isEmpty()) {
                                android.util.Log.w("LessonQuestionFragment", "Invalid video URI for path: ${media.path}")
                                playerView.visibility = View.GONE
                                return@launch
                            }
                            ...
                            exoPlayer.addListener(object : Player.Listener {
                                override fun onPlayerError(error: PlaybackException) {
                                    android.util.Log.e("LessonQuestionFragment", "ExoPlayer error: ${error.message}", error)
                                    ...
                                    if (currentUri.startsWith("http://") || currentUri.startsWith("https://")) {
                                        android.util.Log.w("LessonQuestionFragment", "Supabase video failed, trying asset URI")
                                        val assetUri = media.asUri()
                                        if (assetUri.toString() != currentUri) {
                                            try {
                                                ...
                                                android.util.Log.e("LessonQuestionFragment", "Error loading asset video", e)
                                                playerView.visibility = View.GONE
                                            }
                                        } else {
                                            playerView.visibility = View.GONE
                            ...
                            android.util.Log.e("LessonQuestionFragment", "Error loading video URI: ${e.message}", e)
                            // Fallback to asset URI
                            try {
                                val assetUri = media.asUri()
                                val mediaItem = MediaItem.Builder().setUri(assetUri).build()
                                ...
                                android.util.Log.e("LessonQuestionFragment", "Error loading asset video (fallback)", fallbackError)
                                playerView.visibility = View.GONE
                            }
                        }
                    }
    ...
    fun setOnAnswerListener(listener: (Boolean) -> Unit) {
    ...
    private fun showVideoDialog(media: MediaResource, text: String?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                ...
                if (videoUri == null || videoUri.toString().isEmpty()) {
                    android.util.Log.w("LessonQuestionFragment", "Invalid video URI for dialog: ${media.path}")
                    dialog.dismiss()
                    return@launch
                }
                ...
                dialogPlayer.addListener(object : Player.Listener {
                    override fun onPlayerError(error: PlaybackException) {
                        android.util.Log.e("LessonQuestionFragment", "Dialog ExoPlayer error: ${error.message}", error)
                        // Try asset URI as fallback if Supabase URL failed
                        val currentUri = videoUri.toString()
                        if (currentUri.startsWith("http://") || currentUri.startsWith("https://")) {
                            android.util.Log.w("LessonQuestionFragment", "Supabase video failed in dialog, trying asset URI")
                            val assetUri = media.asUri()
                            if (assetUri.toString() != currentUri) {
                                try {
                                    ...
                                    android.util.Log.e("LessonQuestionFragment", "Error loading asset video in dialog", e)
                                }
                            }
                        }
                    }
                ...
                android.util.Log.e("LessonQuestionFragment", "Error loading video URI for dialog: ${e.message}", e)
                // Fallback to asset URI
                try {
                    val assetUri = media.asUri()
                    val mediaItem = MediaItem.Builder().setUri(assetUri).build()
                    ...
                    android.util.Log.e("LessonQuestionFragment", "Error loading asset video in dialog (fallback)", fallbackError)
                    dialog.dismiss()
                }
            }
        }
